...
while 1:
   #通过学号和日期查询
   print("1.Enter ID and Date query ")
   #通过日期查询
   print("2.Enter Date query")
   #查询本月所有数据并统计患病与未患病数量用柱状图表示
   print("3.Query this month's infomation")
   #查过去7天数据，同上
   print("4.Query infomation in the past 7 days")
   #查上周数据
   print("5.Query last week's infomation")
   print("6.quit")
   ...
   if a==1:
      ...
      sql="select count(*)  \
         from risk,info \
         where DATE_SUB(CURDATE(), INTERVAL 7 DAY) <='2020-03-16' \
         and risk.risk='yes' \
         and risk.id=info.id "
      cursor.execute(sql)
      # 获取所有记录列表
